...
function _getInitHtml(themesPath, bodyClass, cssPath, cssData) {
	...
	_each(cssPath, function(i, path) {
		if (path) {
			arr.push('<link href="../editor/' + path + '" rel="stylesheet" />');
		}
	});
	if (cssData) {
...
_extend(KUploadButton, {
	init : function(options) {
		...
		var html = [
			...
			(options.form ? '<div class="ke-upload-area">' : '<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' + target + '" action="../editor/' + url + '">'),
...
_plugin('core', function(K) {
	...
	self.beforeSetHtml(function(html) {
		...
		.replace(/<a[^>]*name="([^"]+)"[^>]*>(?:<\/a>)?/ig, function(full) {
			...
			return '<img class="ke-anchor" src="../editor/' + self.themesPath + 'common/anchor.gif" data-ke-name="' + escape(attrs.name) + '" />';
		})
