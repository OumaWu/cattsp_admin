...
function _getInitHtml(themesPath, bodyClass, cssPath, cssData) {
	...
	_each(cssPath, function(i, path) {
		if (path) {
			arr.push('<link href="../igs/editor/' + path + '" rel="stylesheet" />');
		}
	});
	if (cssData) {
...
_extend(KUploadButton, {
	init : function(options) {
		...
		var html = [
			...
			(options.form ? '<div class="ke-upload-area">' : '<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' + target + '" action="../igs/editor/' + url + '">'),
...
_plugin('core', function(K) {
	...
	self.beforeSetHtml(function(html) {
		...
		.replace(/<a[^>]*name="([^"]+)"[^>]*>(?:<\/a>)?/ig, function(full) {
			...
			return '<img class="ke-anchor" src="../igs/editor/' + self.themesPath + 'common/anchor.gif" data-ke-name="' + escape(attrs.name) + '" />';
		})
...
KindEditor.plugin('emoticons', function(K) {
	...
	self.clickToolbar(name, function() {
		...
		if (allowPreview) {
			...
			previewImg = K('<img class="ke-preview-img" src="../igs/editor/' + path + startNum + '.gif" />');
		...
		function bindCellEvent(cell, j, num) {
			...
			cell.click(function(e) {
				self.insertHtml('<img src="../igs/editor/' + path + num + '.gif" border="0" alt="" />').hideMenu().focus();
				e.stop();
			});
		}
...
KindEditor.plugin('filemanager', function(K) {
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		var html = [
			...
			// left start
			'<div class="ke-left">',
			'<img class="ke-inline-block" name="moveupImg" src="../igs/editor/' + imgPath + 'go-up.gif" width="16" height="16" border="0" alt="" /> ',
		...
		function createList(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				var iconUrl = imgPath + (data.is_dir ? 'folder-16.gif' : 'file-16.gif'),
					img = K('<img src="../igs/editor/' + iconUrl + '" width="16" height="16" alt="' + data.filename + '" align="absmiddle" />'),
		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				var img = K('<img src="../igs/editor/' + iconUrl + '" width="80" height="80" alt="' + data.filename + '" />');
				if (!data.is_dir || data.has_file) {
...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		var html = [
			...
			'<img class="ke-refresh-btn" src="../igs/editor/' + imgPath + 'refresh.png" width="16" height="16" alt="" style="cursor:pointer;" title="' + lang.resetSize + '" />',
			'</div>',
			//align
			'<div class="ke-dialog-row">',
			'<label style="width:60px;">' + lang.align + '</label>',
			'<input type="radio" name="align" class="ke-inline-block" value="" checked="checked" /> <img name="defaultImg" src="../igs/editor/' + imgPath + 'align_top.gif" width="23" height="25" alt="" />',
			' <input type="radio" name="align" class="ke-inline-block" value="left" /> <img name="leftImg" src="../igs/editor/' + imgPath + 'align_left.gif" width="23" height="25" alt="" />',
			' <input type="radio" name="align" class="ke-inline-block" value="right" /> <img name="rightImg" src="../igs/editor/' + imgPath + 'align_right.gif" width="23" height="25" alt="" />',
			'</div>',
			//title
			'<div class="ke-dialog-row">',
			...
			'<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' + target + '" action="../igs/editor/' + K.addParam(uploadJson, 'dir=image') + '">',
			//file
			'<div class="ke-dialog-row">',
			hiddenElements.join(''),
		...
		refreshBtn.click(function(e) {
			var tempImg = K('<img src="../igs/editor/' + urlBox.val() + '" />', document).css({
				position : 'absolute',
				visibility : 'hidden',
				top : 0,
				left : '-1000px'
...
KindEditor.plugin('insertfile', function(K) {
	...
	self.clickToolbar(name, function() {
		self.plugin.fileDialog({
			clickFn : function(url, title) {
				var html = '<a href="../igs/editor/' + url + '" data-ke-src="' + url + '" target="_blank">' + title + '</a>';
				self.insertHtml(html).hideDialog().focus();
			}
		});
	});
...
K.extend(KSWFUpload, {
	...
	appendFile : function(file) {
		...
		var img = K('<img src="../igs/editor/' + file.url + '" class="ke-img" data-status="' + file.filestatus + '" width="80" height="80" alt="' + file.name + '" />');
		photoDiv.append(img);
